...
const EsportsPage = () => {
  ...
  const handleCreateWallet = () => {
    if (primaryWallet) {
      toast({ title: "Wallet already connected!", description: "Head to tournaments to start competing." });
      navigate('/tournaments');
    } else {
      window.open('https://lobstr.co', '_blank', 'noopener,noreferrer');
      toast({ 
        title: "Create Your Stellar Wallet", 
        description: "Sign up on LOBSTR, then come back and connect your wallet to start competing." 
      });
    }
  };
      ...
      <main className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <section className="arcade-frame p-10 text-center mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-6">
            <Button onClick={handleCreateWallet} className="bg-primary hover:bg-primary/80 text-primary-foreground font-display">
              {primaryWallet ? 'Go To Tournaments' : 'Create Stellar Wallet'}
            </Button>
