...
const Auth = () => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'twitter' | 'twitch') => {
    ...
    try {
      ...
      toast({
        title: "Redirecting...",
        description: `Connecting with ${provider === 'google' ? 'Google' : provider === 'facebook' ? 'Facebook' : provider === 'twitter' ? 'X.com' : 'Twitch'}`,
      });
    } catch (error: any) {
      toast({
