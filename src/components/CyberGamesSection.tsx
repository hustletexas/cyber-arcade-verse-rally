...
const GameCard: React.FC<GameCardProps> = ({ 
  ...
  return (
    <Card 
      className={`relative overflow-hidden cursor-pointer group transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl border-0 ${gradient}`}
    ...
    >
      ...
      {badge && (
        <Badge className={`absolute top-3 right-3 ${badgeColor} text-black font-bold text-xs`}>
          {badge}
        </Badge>
      )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto">
        <GameCard
          ...
          gradient="bg-gradient-to-br from-cyan-900/40 via-gray-900/60 to-gray-900/40 border border-neon-cyan/20"
          route="/games/cyber-match"
          badge="POPULAR"
          badgeColor="bg-neon-cyan"
        />
        ...
        <GameCard
          ...
          description="Simon-style memory game. Watch, remember, repeat the sequence!"
          icon={<Brain className="w-8 h-8 text-neon-purple" />}
          gradient="bg-gradient-to-br from-purple-900/40 via-gray-900/60 to-gray-900/40 border border-neon-purple/20"
