  ...
  return <Card className="arcade-frame">
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {prizes.map(prize => <Card key={prize.id} className="vending-machine hover:scale-105 transition-transform">
              <CardContent className="p-0">
                ...
                <div className="p-6 text-center space-y-4">
                  ...
                  <Button onClick={() => handleRedeemPrize(prize)} disabled={processingPrize === prize.id || !user} className="cyber-button w-full">
                    {processingPrize === prize.id ? "⏳ PROCESSING..." : !user ? "🔐 LOGIN TO REDEEM" : "🎁 REDEEM PRIZE"}
                  </Button>
                </div>
              </CardContent>
            </Card>)}
