...
export const CyberChestPicker: React.FC = () => {
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-col items-center justify-center w-full">
        <img alt="Cyber Chest" className="w-full max-w-lg h-auto filter drop-shadow-[0_0_20px_rgba(255,0,255,0.6)]" src="/lovable-uploads/32650b89-4f1f-4350-a2da-fec37e7df9ac.png" />
      </div>
      <div className="flex items-center justify-center gap-2 mt-2">
        {ownedPassTier && <Badge className={getTierStyles(ownedPassTier).badge}>
            <Trophy className="w-3 h-3 mr-1" />
            {ownedPassTier.charAt(0).toUpperCase() + ownedPassTier.slice(1)} Pass
          </Badge>}
        <Badge className={`${hasUnclaimedChests ? 'bg-neon-green/20 text-neon-green border-neon-green' : 'bg-muted/20 text-muted-foreground border-muted'}`}>
      ...
      <div className="text-center space-y-2">
        {!hasUnclaimedChests ? <div className="bg-card/50 rounded-lg p-4 border border-muted/30">
            ...
            {openedReward ? '✨ Chest opened! Win more games to earn additional chests.' : `🎁 You have ${unclaimedCount} chest${unclaimedCount > 1 ? 's' : ''} to open!`}
          </p>}
      </div>

      {/* Opened Reward Display */}
      {openedReward && selectedChest && <div className="relative rounded-xl overflow-hidden border-2 border-yellow-500/60 bg-gradient-to-br from-yellow-900/30 to-black/60 p-8 text-center animate-scale-in">
          ...
          <img src={selectedChest.image} alt={selectedChest.name} className="w-56 h-56 object-contain mx-auto mb-6" />
          ...
          <Button onClick={handleReset} className="mt-6 cyber-button">
            View Chests
          </Button>
        </div>}

      {/* Chest Cards Grid - Larger images */}
      {!openedReward && <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        return <Card key={chest.id} onClick={() => canOpen && hasUnclaimedChests && !isOpening && handleOpenChest(chest)} className={`
                ...
                <CardContent className="p-0 flex flex-col items-center text-center relative">
                  ...
                  <div className="w-full aspect-[3/4] relative">
                    <img src={chest.image} alt={chest.name} className="w-full h-full object-cover" />
                    
                    {/* Gradient overlay for text readability */}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent" />
                    
                    {/* Lock indicator - bottom left corner, transparent */}
                    {(!canOpenChest(chest) || !hasUnclaimedChests) && <div className="absolute bottom-2 left-2 z-10 flex items-center gap-1.5 bg-black/50 backdrop-blur-sm rounded-md px-2 py-1">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 text-center">
                    ...
                    {isEligible && <Button size="sm" disabled={isOpening} className={`w-full bg-gradient-to-r ${chest.tier === 'legendary' ? 'from-yellow-500 to-orange-500' : chest.tier === 'epic' ? 'from-purple-500 to-pink-500' : chest.tier === 'rare' ? 'from-blue-500 to-cyan-500' : 'from-green-500 to-emerald-500'} text-white hover:opacity-90`}>
                        {isCurrentlyOpening ? <>
      ...
      {/* Pass Upgrade Hint */}
      {ownedPassTier && ownedPassTier !== 'legendary' && !openedReward && <p className="text-xs text-center text-muted-foreground">
