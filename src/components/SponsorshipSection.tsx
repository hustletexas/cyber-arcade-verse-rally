...
const sponsorshipIncludes = [
  ...
  { icon: Megaphone, text: 'Logo placement on the platform or promotional materials' },
  { icon: Trophy, text: 'Sponsored tournaments or challenges' },
  { icon: Users, text: 'Community engagement opportunities' },
];

const sponsorTiers = [
  { label: 'Community Sponsor', icon: Users, color: 'text-neon-cyan', border: 'border-neon-cyan/30', glow: 'shadow-[0_0_15px_hsl(var(--neon-cyan)/0.15)]' },
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <Card className="holographic bg-card/40">
          <CardContent className="p-5 sm:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {sponsorshipIncludes.map((item, i) => (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            {sponsorTiers.map((tier) => (
              <Card
                key={tier.label}
                className={`bg-card/30 backdrop-blur-sm border ${tier.border} ${tier.glow} hover:scale-105 transition-transform duration-300`}
              >
        ...
        <Card className="holographic bg-card/40 border-neon-pink/20">
          <CardContent className="p-5 sm:p-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Button
                asChild
                className="cyber-button px-6 py-3 text-sm sm:text-base w-full sm:w-auto"
              >
        ...
        <div className="text-center">
          <p className="text-muted-foreground/60 text-xs max-w-lg mx-auto leading-relaxed">
            Community donations via USDC/XLM are accepted for platform support. Donations do not
            confer any ownership, equity, or financial return. Transaction transparency is maintained
            on-chain.
          </p>
        </div>
      </div>
