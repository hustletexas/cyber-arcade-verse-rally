...
export const Marketplace = () => {
  ...
  const filteredNFTs = filter === 'all' ? mockNFTs : mockNFTs.filter(nft => nft.rarity.toLowerCase() === filter.toLowerCase());
  ...
  return <Card className="arcade-frame">
      ...
      <CardContent>
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-4 items-center justify-between">
            <div className="flex gap-4 items-center">
              <div className="flex gap-2 flex-wrap">
                <Button size="sm" variant={filter === 'all' ? 'default' : 'outline'} onClick={() => setFilter('all')} className={filter === 'all' ? 'cyber-button' : 'border-neon-cyan text-neon-cyan'}>
                  All
                </Button>
                <Button size="sm" variant={filter === 'legendary' ? 'default' : 'outline'} onClick={() => setFilter('legendary')} className={filter === 'legendary' ? 'cyber-button' : 'border-neon-purple text-neon-purple'}>
                  Legendary
                </Button>
                <Button size="sm" variant={filter === 'epic' ? 'default' : 'outline'} onClick={() => setFilter('epic')} className={filter === 'epic' ? 'cyber-button' : 'border-neon-pink text-neon-pink'}>
                  Epic
                </Button>
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="icon" onClick={scrollNext} className="border-neon-pink/50 text-neon-pink hover:bg-neon-pink/20">
            ...
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex gap-4">
                {(filter === 'all' ? mockNFTs : mockNFTs.filter(nft => nft.rarity.toLowerCase() === filter.toLowerCase())).map(nft => <div key={nft.id} className="flex-[0_0_100%] min-w-0 md:flex-[0_0_50%] lg:flex-[0_0_33.333%] pl-0 first:pl-0 [perspective:1000px] h-[580px]">
                        ...
                        <CardContent className="p-0 h-full flex flex-col">
                          ...
                          <div className="p-4 space-y-2 flex-1">
                            <div className="flex justify-between items-start">
                              ...
                              <Badge className={`text-xs ${nft.rarity === 'Legendary' ? 'bg-yellow-500 text-black' : nft.rarity === 'Epic' ? 'bg-neon-purple text-white' : nft.rarity === 'Rare' ? 'bg-blue-500 text-white' : 'bg-gray-500 text-white'}`}>
                                {nft.rarity}
                              </Badge>
                            </div>
                        ...
                        <CardContent className="p-4 h-full flex flex-col justify-between bg-gradient-to-br from-neon-purple/10 via-background to-neon-cyan/10">
                          <div className="space-y-3">
                            <div className="flex justify-between items-start">
                              ...
                              <Badge className={`${nft.rarity === 'Legendary' ? 'bg-yellow-500 text-black animate-pulse' : nft.rarity === 'Epic' ? 'bg-neon-purple text-white' : nft.rarity === 'Rare' ? 'bg-blue-500 text-white' : 'bg-gray-500 text-white'}`}>
                                {nft.rarity}
                              </Badge>
                            </div>
          ...
          {/* Quick Cart Access */}
          {getTotalItems() > 0 && <Card className="holographic p-6">
              ...
              <div className="flex gap-4">
                <Button onClick={() => setIsOpen(true)} className="flex-1 cyber-button">
                  🚀 VIEW CART & CHECKOUT
                </Button>
              </div>
            </Card>}
